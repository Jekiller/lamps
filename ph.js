(function () {
    ...
    var Api = {
        ...
        request: function(method, success, error) {
            ...
            Network.silent(final_url, function(str) {
                if (str.length < 500) Lampa.Noty.show('PH: Ответ слишком короткий. Проверьте прокси.');
                if (str.indexOf('captcha') > -1) Lampa.Noty.show('PH: Капча! Смените прокси или IP.');
                success(str);
            }, function(a, c) {
                error(a, c);
            }, params);
        ...
        list: function(html) {
            ...
            elements.forEach(function(el) {
                ...
                if (linkEl && imgEl) {
                    ...
                    var img = imgEl.getAttribute('data-mediumthumb') || imgEl.getAttribute('data-src') || imgEl.getAttribute('data-thumb_url') || imgEl.src;
    ...
    function Component(object) {
        ...
        comp.load = function(endpoint) {
            ...
            Api.request(endpoint, function(html) {
                ...
                if (data.results.length === 0) {
                    var empty = Lampa.Template.get('empty', { title: 'Пусто', descr: 'Нет видео. Проверьте прокси.' });
        ...
        comp.append = function(data) {
            ...
            data.results.forEach(function(element) {
                var card = Lampa.Template.get('card', { title: element.title, release_year: element.info });
                ...
                card.on('hover:enter', function() {
                    Lampa.Activity.push({ url: element.url, title: element.title, component: 'ph_mod_view', page: 1 });
                });
    ...
    function View(object) {
        ...
        comp.start = function() {
            ...
            Api.request(object.url, function(html) {
                ...
                if (data.videos && data.videos.length) {
                    ...
                    Lampa.Noty.show('Видео не найдено. Нужен Premium или другой прокси.');
        ...
        comp.show = function(data) {
            var desc = Lampa.Template.get('description', { title: data.title, descr: 'Нажмите Play' });
            ...
            btn.on('hover:enter', function() {
                Lampa.Select.show({
                    ...
                    items: data.videos,
                    onSelect: function(v) {
                        var vid = { title: data.title, url: v.url, timeline: { hash: Lampa.Utils.uid(data.title) } };
    ...
    function startPlugin() {
        ...
        Lampa.SettingsApi.addParam({
            ...
            param: { name: 'ph_domain', type: 'select', values: { 'https://rt.pornhub.com': 'RT (Russia)', 'https://www.pornhub.com': 'WWW (Global)' }, default: 'https://rt.pornhub.com' },
            field: { name: 'Домен PH', description: 'Выберите зеркало' },
            onChange: function(value) { Settings.set('domain', value); }
        });
        Lampa.SettingsApi.addParam({
            component: 'ph_settings',
            param: { name: 'ph_proxy_type', type: 'select', values: { 'corsproxy': 'CorsProxy.io', 'allorigins': 'AllOrigins', 'custom': 'Custom' }, default: 'corsproxy' },
            field: { name: 'Тип Прокси', description: 'Метод обхода' },
            onChange: function(value) { Settings.set('proxy_type', value); }
        });
        Lampa.SettingsApi.addParam({
            component: 'ph_settings',
            param: { name: 'ph_custom_proxy', type: 'input', default: '' },
            field: { name: 'Свой прокси URL', description: 'https://myproxy.com/?url=' },
            onChange: function(value) { Settings.set('custom_proxy', value); }
        });

        // Добавление кнопки в меню (Универсальный метод)
        var btn = $('<li class="menu__item selector" data-action="ph_mod"><div class="menu__ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M2.5 12a9.5 9.5 0 1 1 19 0 9.5 9.5 0 0 1-19 0Z"/><path d="M10 8l6 4-6 4V8z"/></svg></div><div class="menu__text">PH Premium</div></li>');
        ...
        // Добавляем кнопку в список меню (ищем правильный селектор)
        var menuList = $('.menu .menu__list');
        if (menuList.length) {
            menuList.append(btn);
        } else {
            // Фолбек для очень старых версий
            $('.activity__menu .activity__menu-list').append(btn);
        }
