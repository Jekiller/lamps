...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  ...
  function component(object) {
    ...
    this.toPlayElement = function(file) {
		// Додаємо сповіщення
        Lampa.Noty.show('Безкоштовно від lampaua.mooo.com (t.me/bazarnetua) Не платіть шахраям!', {time: 5000});  
      var play = {
        title: file.title,
        url: file.url,
...
// --- ПОЧАТОК: ФУНКЦІЯ З ІНТЕРВАЛОМ МІЖ ПОКАЗАМИ ---
  function showFreeWarning() {
      // НАЛАШТУВАННЯ: Мінімальний інтервал у хвилинах
      var intervalMinutes = 180; // Наприклад, 60 хвилин між показами
      
      // 1. Отримуємо поточні дані
      var now = Date.now(); // Поточний час у мілісекундах
      var today = new Date().toDateString(); // Поточна дата
      var stats = Lampa.Storage.get('lampac_free_warning_stats', { date: '', count: 0, last_time: 0 });

      // 2. Якщо настав новий день - скидаємо лічильник
      if (stats.date !== today) {
          stats.date = today;
          stats.count = 0;
          stats.last_time = 0; // Скидаємо час останнього показу
      }

      // 3. Перевірки:
      // А) Якщо вже показали 2 рази за сьогодні - виходимо
      if (stats.count >= 2) return;
      
      // Б) Якщо з моменту останнього показу пройшло менше часу, ніж вказано в налаштуваннях
      // (але тільки якщо це не перший показ за день)
      if (stats.count > 0 && (now - stats.last_time) < (intervalMinutes * 60 * 1000)) {
          return; 
      }

      // 4. Якщо перевірки пройшли - показуємо вікно
      if ($('.lampac-free-warning').length) return; // Захист від дублювання
      
      var textTitle = "УВАГА! ЦЕЙ СЕРВІС БЕЗКОШТОВНИЙ!";
      var textBody = "Джерело: lampaua.mooo.com.<br>Якщо ви заплатили за цей плагін гроші — ВАС ОШУКАЛИ!";
      ...
      // 5. Оновлюємо статистику (збільшуємо лічильник і записуємо час)
      stats.count++;
      stats.last_time = now;
      Lampa.Storage.set('lampac_free_warning_stats', stats);
